...
// Function to load population density data
// Global variable to track if population data is loaded
...
async function loadPopulationData() {
    ...
    if (statusDiv && statusDiv.style.display === 'block') {
        statusDiv.innerHTML = 'Loading population data...';
    }
    ...
    try {
        ...
        const response = await fetch(`/maps/api/population-density/`);
        ...
        if (heatData.length > 0 && typeof L.heatLayer === 'function') {
            ...
            // Set the flag to indicate data is loaded
            populationDataLoaded = true;
            
            // Update status if needed
            if (statusDiv && statusDiv.style.display === 'block') {
                statusDiv.innerHTML = 'Population data loaded successfully';
            }
        ...
        // Update status if needed
        if (statusDiv && statusDiv.style.display === 'block') {
            statusDiv.innerHTML = 'Error loading population data';
        }
